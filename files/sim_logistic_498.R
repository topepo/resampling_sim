...
test_res <-
  ...
  mutate(seed = 498, model = "logistic")
...
for (iters in B) {
  ...
  tmp_res <-
    ...
    mutate(times = iters, seed = 498, model = "logistic")
  ...
  tmp_rand <-
    ...
    mutate(times = iters, seed = 498, model = "knn")
...
for (pct in retain_pct) {
  ...
  tmp_res <-
    ...
    collect_metrics()%>%
    mutate(retain = pct, estimator = "standard", seed = 498, model = "logistic")
...
resub <-
  ...
  pluck(".metrics") %>%
  pluck(1) %>%
  mutate(estimator = "standard", seed = 498, model = "logistic")
